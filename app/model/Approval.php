...
class Approval {
    ...
    public function create(array $data): int {
        $stmt = $this->pdo->prepare("
            INSERT INTO approvals (form_id, approver_id, sequence, status)
            VALUES (:form_id, :approver_id, :sequence, 'pending')
        ");
        $stmt->execute([
    ...
    private function log(int $formId, int $level, string $action, int $actorId, string $resultStatus, string $remarks): void {
        $this->pdo->prepare("
            INSERT INTO audit_logs (
                performed_by, action, entity_type, entity_id, old_values, new_values, ip_address
            )
            VALUES (
                :actor,
                :action,
